...
for domain_name in SUITE:
    ...
    run.add_command(
        "main_script_pairwise_wl",
        ["python", "-u", "{main_script}", "pairwise-wl", "--data-path", f"{domain_name}", "--mark-true-goal-atoms", "--max-num-states", "10000"],
        time_limit=TIME_LIMIT,
        memory_limit=MEMORY_LIMIT,
    )
    # AbsoluteReport needs the following properties:
    ...
    run.add_command(
        "main_script_pairwise_wl",
        ["python", "-u", "{main_script}", "pairwise-wl", "--data-path", f"{domain_name}", "--ignore-counting", "--max-num-states", "10000"],
        time_limit=TIME_LIMIT,
        memory_limit=MEMORY_LIMIT,
    )
    # AbsoluteReport needs the following properties:
    ...
    run.add_command(
        "main_script_pairwise_wl",
        ["python", "-u", "{main_script}", "pairwise-wl", "--data-path", f"{domain_name}", "--mark-true-goal-atoms", "--ignore-counting", "--max-num-states", "10000"],
        time_limit=TIME_LIMIT,
        memory_limit=MEMORY_LIMIT,
    )
    # AbsoluteReport needs the following properties:
